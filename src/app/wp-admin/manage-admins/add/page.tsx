...
export default function AddAdminPage() {
  ...
  useEffect(() => {
    if (!avatarFile) return setPreviewUrl(null);
  ...
  useEffect(() => {
    if (!signatureFile) return setSignaturePreview(null);
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      const [avatarData, signatureData] = await Promise.all([
        avatarFile ? fileToDataUrl(avatarFile) : Promise.resolve<string | null>(null),
        signatureFile ? fileToDataUrl(signatureFile) : Promise.resolve<string | null>(null),
      ] as [Promise<string | null>, Promise<string | null>]);

      const id = String(Date.now());
      const adminObj: any = { ...form, id, joined: new Date().toISOString().slice(0,10) };
      if (avatarData) adminObj.avatar = avatarData;
      if (signatureData) adminObj.signature = signatureData;
      addAdmin(adminObj);
        ...
        <header className="sticky top-0 bg-gray-50/80 backdrop-blur-sm z-10 py-4 mb-6">
          <div className="flex justify-between items-center">
            <div>
              ...
              <p className="text-sm text-gray-500">Create a new admin and assign company details (UI only)</p>
            </div>
              ...
              <div className="space-y-6">
                ...
                <div className="border border-gray-200 rounded-lg p-4 space-y-4">
                  ...
                  <div className="space-y-2">
                    <div className="flex items-center justify-between"><span className="text-sm">Enable e-invoicing</span><ToggleSwitch enabled={form.enableEInvoicing} onChange={(v) => handleChange('enableEInvoicing', v)} /></div>
                    <div className="flex items-center justify-between"><span className="text-sm">Enable TDS</span><ToggleSwitch enabled={form.enableTds} onChange={(v) => handleChange('enableTds', v)} /></div>
                    <div className="flex items-center justify-between"><span className="text-sm">Enable TCS</span><ToggleSwitch enabled={form.enableTcs} onChange={(v) => handleChange('enableTcs', v)} /></div>
                  </div>
                </div>

                <div className="space-y-3">
                  <FormField label="Status">
                    <select value={form.status} onChange={(e) => handleChange('status', e.target.value)} className="w-full border rounded-md px-3 py-2">
                  ...
                  <FormField label="Plan">
                    <select value={form.plan} onChange={(e) => handleChange('plan', e.target.value)} className="w-full border rounded-md px-3 py-2">
