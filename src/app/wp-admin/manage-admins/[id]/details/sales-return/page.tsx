...
export default function SalesReturnPage() {
  ...
  const [customerSelect, setCustomerSelect] = useState<string>('all');
  ...
  const renderCell = (row: any, k: string, productsList: any[]) => {
    ...
    // Format customer/party
    if ((k === 'customer' || k === 'selectedCustomer' || k === 'selectedParty' || k === 'party') && v && typeof v === 'object') {
      const p: any = v;
      const name = p.name || p.partyName || p.customerName || p.displayName || '—';
      const phone = p.mobileNumber || p.phone || p.contact || '';
      const gst = p.gstin || p.taxNumber || '';
      const billing = p.billingAddress || p.address || p.billing || '';
      const shipping = p.shippingAddress || p.shipping || '';
      const balance = p.balance ?? p.openingBalance ?? p.currentBalance ?? null;
      return (
  ...
  useEffect(() => {
    ...
    async function load() {
      setLoading(true);
      try {
        const res = await fetch(`/api/admin/new_sale_return?businessId=${encodeURIComponent(id)}`, { credentials: 'include' });
        if (!res.ok) {
          console.error('failed to fetch sale returns', await res.text());
  ...
  useEffect(() => {
    ...
    async function loadProducts() {
      try {
        ...
        if (!res.ok) {
          console.warn('failed to fetch products for sales return details', await res.text());
        ...
        console.warn('error loading products for sales return details', e);
        if (mounted) setProducts([]);
      }
    }
    if (id) loadProducts();
    ...
    <div className="mt-6">
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
        <div className="bg-white p-4 rounded-lg shadow-sm border">
          <div className="text-sm text-gray-500">Sales Returns</div>
      ...
      <div className="bg-white rounded-lg shadow-sm border p-4">
        <div className="flex items-center justify-between mb-3">
          <h3 className="text-lg font-semibold">Sales Return</h3>
        </div>
        <div>
          {/* Filters */}
          <div className="mb-4 grid grid-cols-1 md:grid-cols-6 gap-2">
            ...
            <div>
              ...
              <select value={customerSelect} onChange={e => setCustomerSelect(e.target.value)} className="mt-1 block w-full border rounded px-2 py-1">
                <option value="all">All</option>
                {Array.from(new Set(returnsList.map(s => ((s.customer && (s.customer.name || s.customer.partyName)) || s.customerName || '').trim()).filter(Boolean))).map(p => (
            ...
            <div>
              ...
              <input placeholder="Search return, customer, product..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} className="mt-1 block w-full border rounded px-2 py-1" />
            </div>
            <div>
          ...
          <div className="flex gap-2 mb-4">
            <button onClick={() => { setStartDate(''); setEndDate(''); setCustomerSelect('all'); setPaymentStatusFilter('all'); setMinAmount(''); setMaxAmount(''); setProductSelect('all'); setSearchQuery(''); }} className="px-3 py-1 bg-gray-100 rounded">Clear</button>
          </div>
              ...
              <TableBody>
                ...
                {(() => {
                  const applyFilters = (arr: any[]) => arr.filter((s: any) => {
                    ...
                    if (customerSelect && customerSelect !== 'all') {
                      const name = ((s.customer && (s.customer.name || s.customer.partyName)) || s.customerName || '').trim(); if (name !== customerSelect) return false;
                    }
                    if (searchQuery) {
                      const q = searchQuery.toLowerCase();
                      const refHit = (s.returnNo || s._id || '').toString().toLowerCase().includes(q);
                      const custHit = (((s.customer && (s.customer.name || s.customer.partyName)) || s.customerName || '') + '').toLowerCase().includes(q);
                      ...
                      if (!(refHit || custHit || itemsHit)) return false;
                    }
