...
import { getAllNewPurchases } from '@/controllers/newPurchaseController';
...
export const GET = asyncHandler(async (req: NextRequest) => {
  ...
  if (authResult instanceof NextResponse) {
    ...
    console.warn('/api/admin/new_purchase accessed in public mode (unauthenticated)');
  } else {
    user = authResult;
    if (user.role !== 'superadmin' && allowPublic(req)) {
      ...
      console.warn('/api/admin/new_purchase accessed in public mode (authenticated non-superadmin)');
    }
  }
  ...
  let docs = await getAllNewPurchases(syntheticUser);

  // fallback: if no docs and businessIdParam might be a user id, resolve user's business and try again
  if ((!docs || docs.length === 0) && businessIdParam) {
    try {
      ...
      if (maybeUser && maybeUser.business) {
        ...
        const altDocs = await getAllNewPurchases(altUser);
        if (altDocs && altDocs.length > 0) docs = altDocs;
      }
    } catch (e) {
      console.warn('admin/new_purchase: fallback user->business lookup failed', e);
    }
  }

  // Enrich purchase items with product fields (lookup by productId or sku) so client can render friendly item info
  try {
    if (docs && docs.length > 0) {
      ...
      for (const d of docs) {
        ...
        for (const it of d.items) {
          if (!it) continue;
          const pid = it.productId || it.product || it._id || null;
    ...
    console.warn('admin/new_purchase: product enrichment failed', e);
  }

  // Build a fields array to help dynamic frontends render columns
  try {
    ...
    const preferred = ['invoiceNumber','invoiceNo','invoiceDate','createdAt','selectedParty','partyName','items','paymentStatus','payment_status','totalAmount','invoiceAmount','amountReceived','receivedAmount','balanceAmount','balance','additionalCharges'];
