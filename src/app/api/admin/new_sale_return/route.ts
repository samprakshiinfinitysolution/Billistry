...
import { getAllNewSaleReturns } from '@/controllers/newSaleReturnController';
...
export const GET = asyncHandler(async (req: NextRequest) => {
  ...
  if (authResult instanceof NextResponse) {
    ...
    console.warn('/api/admin/new_sale_return accessed in public mode (unauthenticated)');
  } else {
    user = authResult;
    if (user.role !== 'superadmin' && allowPublic(req)) {
      ...
      console.warn('/api/admin/new_sale_return accessed in public mode (authenticated non-superadmin)');
    }
  }
  ...
  let docs = await getAllNewSaleReturns(syntheticUser);
  if ((!docs || docs.length === 0) && businessIdParam) {
    try {
      ...
      if (maybeUser && maybeUser.business) {
        ...
        const altDocs = await getAllNewSaleReturns(altUser);
        if (altDocs && altDocs.length > 0) docs = altDocs;
      }
    } catch (e) {
      console.warn('admin/new_sale_return: fallback user->business lookup failed', e);
    }
  }
