...
const CreatePurchaseReturnInvoicePage = () => {
    ...
    // When a return is linked to an original purchase, lock the form
    ...
    useEffect(() => {
        // If the user clears the LinkToInvoice input, unlock the form and clear invoice-populated fields
        if (!searchInvoiceTerm) {
            if (isLinked) {
                ...
                // reset invoiceDate to today for a standalone return
            ...
            try {
                ...
                if (data && data.success && Array.isArray(data.data)) {
                    const mapped: Invoice[] = data.data.map((d: any) => ({
                        ...
                        // include party id so the UI can filter by selected vendor/party
        ...
        try {
            ...
            (async () => {
                try {
                    ...
                    // Additional charges
                    setAdditionalCharges(Array.isArray(d.additionalCharges) ? d.additionalCharges.map((c: any, i: number) => ({ id: c.id ?? i, name: c.name || '', amount: String(c.amount || ''), taxType: c.taxType || 'no-tax' })) : []);
                    ...
                    // If this return was linked to an original purchase, try to show that invoice in the LinkToInvoice input
                    try {
                        const orig = d.originalPurchase || d.original_purchase || d.originalPurchaseId;
                        if (orig) {
                            ...
                            if (found) {
                                ...
                                try {
                                    const r = await fetch(`/api/new_purchase/${orig}`);
                                    if (r.ok) {
                                        const jb = await r.json().catch(() => null);
                                        const sd = jb?.data;
                                        if (sd) setSearchInvoiceTerm(sd.invoiceNo || (sd.invoiceNumber ? String(sd.invoiceNumber) : ''));
                                        if (sd) setIsLinked(true);
                                    }
                                } catch(e) {}
                            }
                        }
        ...
        try {
            ...
            if (data && data.success && data.data) {
                ...
                if (Array.isArray(d.items)) {
                    ...
                    setItems(loadedItems);
                    // lock the form after populating from a linked purchase invoice
                    setIsLinked(true);
                }
                if (d.selectedParty) {
                    const sp = d.selectedParty;
    ...
    const handleSave = async (shouldRedirect: boolean) => {
        try {
            ...
            // Validation: require party and items
            if (!selectedParty) {
                toast.error('Please select a party before saving the purchase return');
                return;
            }
            if (!items || items.length === 0) {
                toast.error('Please add at least one item before saving the purchase return');
                return;
            }
            ...
            // Detect edit mode using URL param (keeps parity with existing code)
            const params = new URLSearchParams(window.location.search);
            ...
            if (!res || !res.ok) {
                ...
                // Save & New: compute next formatted invoice number and clear form
                ...
                toast.success('Purchase return saved. Ready for new return.');
            }
        } catch (e) {
                ...
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3">
                    <div className="flex justify-between items-center">
                        <div className="flex items-center gap-4">
                            <Button variant="ghost" size="icon" className="text-gray-600 cursor-pointer hover:bg-gray-100 rounded-md p-2" onClick={() => router.push('/dashboard/return/purchase/purchase-return-data')}>
                                <ArrowLeft className="h-5 w-5" />
                            </Button>
                            <h1 className="text-xl font-semibold text-gray-800">Create Purchase Return</h1>
                        </div>
                        <div className="flex items-center gap-2">
                            <Button variant="outline" onClick={() => setIsSettingsOpen(true)} className="bg-white border border-gray-300 text-gray-700 hover:bg-gray-50 px-3 py-1.5 rounded-md cursor-pointer">
                            ...
                            <>
                                <Button onClick={async () => await handleSave(false)} className="bg-white border border-indigo-600 text-indigo-600 hover:bg-indigo-50 px-4 py-2 rounded-md cursor-pointer">
                                    {saving ? 'Saving…' : 'Save & New'}
                                </Button>
                                <Button onClick={async () => await handleSave(true)} className="bg-indigo-600 text-white font-semibold hover:bg-indigo-700 px-8 py-2 rounded-md shadow-md cursor-pointer">
                                    {saving ? 'Saving…' : 'Save'}
                                </Button>
                            </>
                        </div>
                             ...
                             <div className="flex flex-col sm:flex-row gap-4">
                                <div className="w-full sm:w-64">
                                    ...
                                    <div className="flex items-center gap-2 justify-end">
                                        <Input id="invoiceNo" type="text" value={returnInvoiceNo || `PUR-${String(invoiceNumber).padStart(5, '0')}`} readOnly aria-readonly={true} tabIndex={-1} className="text-right w-44 cursor-not-allowed bg-gray-100"/>
                                    </div>
                                </div>
                                 <div className="w-full sm:w-64">
                            ...
                            <tbody>
                                {items.map((item, index) => (
                                    <tr key={item.id} className={`border-b hover:bg-gray-50 ${index % 2 === 0 ? 'bg-white' : 'bg-gray-50'}`}>
                                        ...
                                        <td className="px-2 py-2"><Input type="text" placeholder="Item Name" value={item.name} readOnly={isLinked} onChange={e => { if (!isLinked) handleItemChange(item.id, 'name', e.target.value); }} className={isLinked ? 'bg-gray-100 cursor-not-allowed' : ''} /></td>
                                        <td className="px-2 py-2"><Input type="text" placeholder="HSN" value={item.hsn} readOnly={isLinked} onChange={e => { if (!isLinked) handleItemChange(item.id, 'hsn', e.target.value); }} className={isLinked ? 'bg-gray-100 cursor-not-allowed' : ''} /></td>
                                        ...
                                        <td className="px-2 py-2"><Input type="number" placeholder="0.00" value={item.price} readOnly={isLinked} onChange={e => { if (!isLinked) handleItemChange(item.id, 'price', e.target.value); }} className={isLinked ? 'bg-gray-100 cursor-not-allowed' : ''} /></td>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-x-16">
                            ...
                            {showNotesInput ? (
                                <div className="relative">
                                    ...
                                    <Textarea
                                        ...
                                        rows={3}
                                        className={`bg-gray-100 p-3 rounded-md border-none focus-visible:ring-0 p-0 ${isLinked ? 'cursor-not-allowed' : ''}`}
                                    />
                                    <Button variant="ghost" size="icon" className={`absolute top-2 right-2 text-gray-400 ${isLinked ? '' : 'cursor-pointer hover:text-gray-600'}`} onClick={!isLinked ? () => setShowNotesInput(false) : undefined} disabled={isLinked}>
                                        <X className="h-4 w-4" />
                                    </Button>
                                </div>
                            ) : (
                            <Button variant="link" className={`${isLinked ? 'text-gray-400 p-0' : 'text-blue-600 cursor-pointer p-0 hover:underline'}`} onClick={!isLinked ? () => setShowNotesInput(true) : undefined} disabled={isLinked}><Plus className="mr-1 h-4 w-4" /> Add Notes</Button>)}
                           ...
                           <div className="relative">
                               <label className="text-sm font-medium text-gray-500 mb-1 block">Terms and Conditions</label>
                                         <div className="bg-gray-100 p-3 rounded-md">
                                             <Textarea value={terms} readOnly={isLinked} onChange={(e) => { if (!isLinked) setTerms(e.target.value); }} rows={3} className={`bg-transparent border-none focus-visible:ring-0 p-0 ${isLinked ? 'cursor-not-allowed' : ''}`}/>
                                         </div>
                                         <Button variant="ghost" size="icon" className={`absolute top-7 right-2 text-gray-400 ${isLinked ? '' : 'cursor-pointer hover:text-gray-600'}`} onClick={!isLinked ? () => setTerms('') : undefined} disabled={isLinked}>
                                                <X className="h-4 w-4" />
                                         </Button>
                           </div>
                        </div>
                        ...
                        <div className="space-y-4 mt-6 md:mt-0 sticky top-24 self-start">
                            <div className="space-y-2">
                                ...
                                <Button variant="link" className={`${isLinked ? 'text-gray-400 cursor-not-allowed p-0' : 'text-blue-600 cursor-pointer p-0 hover:underline'}`} onClick={!isLinked ? handleAddCharge : undefined} disabled={isLinked}>
                            ...
                            <div className="flex justify-between items-center text-sm">
                                {!showDiscountInput ? (
                                    <>
                                            <Button variant="link" className={`${isLinked ? 'text-gray-400 cursor-not-allowed p-0' : 'text-blue-600 cursor-pointer p-0 hover:underline'}`} onClick={!isLinked ? () => setShowDiscountInput(true) : undefined} disabled={isLinked}>
                            ...
                            <div className="flex justify-between items-center text-sm">
                                <label htmlFor="autoRoundOff" className={`flex items-center gap-2 text-gray-600 ${isLinked ? 'cursor-not-allowed' : 'cursor-pointer'}`}>
                                    <Checkbox id="autoRoundOff" checked={autoRoundOff} disabled={isLinked} onChange={(e) => {
                                        if (isLinked) return;
