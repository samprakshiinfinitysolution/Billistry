...
interface InvoiceItem {
    id: number;
    itemId: string; // This will store the Product _id
    name: string;
    hsn: string;
    qty: number;
    price: number;
...
const CreateSalesInvoicePage = () => {
    ...
    const handleAddItemFromModal = (itemToAdd: ItemData, quantity: number) => {
        ...
        const newItem: InvoiceItem = {
            ...
            itemId: itemToAdd._id, // Store the product ID
            name: itemToAdd.name,
    ...
    const handleSave = async () => {
        ...
        try {
            const payload = {
                billTo: selectedParty._id,
                date: invoiceDate,
                items: items.map(item => ({
                    item: item.itemId, // Use the stored product ID
                    quantity: item.qty,
                    rate: item.price,
                })),
                paymentStatus: isFullyPaid ? 'cash' : 'unpaid', // Example logic, you can refine this
                notes: notes,
                discountType: lastDiscountInput === 'percent' ? 'percent' : 'flat',
                discountValue: lastDiscountInput === 'percent' ? parseFloat(discountPercentStr) || 0 : parseFloat(discountFlatStr) || 0,
                taxRate: 0, // This seems to be an overall tax rate, which your UI doesn't have. Sending 0 for now.
                // The backend calculates amounts, so we don't need to send them.
            };

            const response = await axios.post('/api/sale', payload, { withCredentials: true });

            if (response.data.success) {
                alert('Sales invoice created successfully!');
                router.push('/dashboard/sale'); // Redirect to the sales list page
            } else {
                ...
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3">
                    <div className="flex justify-between items-center">
                        ...
                        <div className="flex items-center gap-2">
                            ...
                            <Button onClick={handleSave} disabled={isSaving} className="bg-indigo-600 text-white font-semibold hover:bg-indigo-700 px-4 py-2 disabled:bg-indigo-400">
                                {isSaving ? 'Saving...' : 'Save Sales Invoice'}
                            </Button>
                        </div>
                    </div>
                </div>
