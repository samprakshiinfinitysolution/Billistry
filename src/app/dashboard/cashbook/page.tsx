...
export default function CashbookPage(): JSX.Element {
  ...
  async function fetchData() {
    try {
      ...
      json.entries = json.entries.map((e) => ({
        ...e,
        mode: (e.mode || "").toString().toLowerCase() === "online" ? "Online" : "Cash",
      }));
      setData(json);
    } catch (err) {
      console.error(err);
  ...
  const filteredEntries = useMemo(() => {
    ...
    return data.entries.filter((e) => {
      ...
      if (search && !`${e.description} ${e.mode}`.toLowerCase().includes(search.toLowerCase())) return false;
...
function exportPDF() {
  ...
  function openAddModal(defaultType: "IN" | "OUT" = "IN") {
    setEntryForm({ type: defaultType, amount: "", mode: "Cash", description: "" });
  ...
  async function handleEditSubmit(e: React.FormEvent) {
    e.preventDefault();
    if (!selectedEntry) return alert("No entry selected");
    const amountNum = Number(entryForm.amount);
    if (!amountNum || amountNum <= 0) {
      alert("Amount must be greater than 0");
      return;
  ...
  async function handleDeleteConfirm() {
    ...
    setSubmitting(true);
    try {
      const res = await fetch(`/api/cashbook/${selectedEntry._id}`, { method: "DELETE" });
      if (!res.ok) {
        const text = await res.text();
  ...
  if (!data) return <div className="p-6 text-center text-gray-500">No data found</div>;
    ...
    <div className="p-6">
      <div className="flex justify-between items-center mb-4">
        ...
        <div className="flex gap-2">
          <button onClick={() => openAddModal("IN")} className="bg-green-600 text-white px-3 py-2 rounded">+ Add IN</button>
          <button onClick={() => openAddModal("OUT")} className="bg-red-600 text-white px-3 py-2 rounded">+ Add OUT</button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-6 gap-4 mb-6">
        {summaryItems.map((item) => (
          <div key={item.label} className="bg-white shadow rounded-lg p-4 text-center border">
      ...
      <div className="bg-white shadow rounded-lg p-4 mb-4 flex flex-wrap gap-3 items-end">
        ...
        <div className="flex-1">
          ...
          <input type="text" value={search} onChange={(e) => { setSearch(e.target.value); setPage(1); }} placeholder="Search description or mode" className="border rounded px-3 py-2 w-full" />
        </div>

        <div className="ml-auto flex gap-2">
          <button onClick={exportCSV} className="bg-gray-200 px-3 py-2 rounded hover:bg-gray-300">Export CSV</button>
          <button onClick={exportPDF} className="bg-gray-200 px-3 py-2 rounded hover:bg-gray-300">Export PDF</button>
        </div>
      </div>
        ...
        <table className="min-w-full text-sm">
          ...
          <tbody>
            {paginatedEntries.length === 0 ? (
              <tr>
                <td colSpan={6} className="text-center p-4 text-gray-500">No entries found</td>
              </tr>
            ) : (
              paginatedEntries.map((e) => (
                <tr key={e._id} className="border-t">
                  <td className="p-3">{new Date(e.createdAt).toLocaleString()}</td>
                  <td className={`p-3 font-semibold ${e.type === "IN" ? "text-green-600" : "text-red-600"}`}>{e.type}</td>
      ...
      {totalPages > 1 && (
        <div className="flex justify-center mt-4 gap-2">
          <button disabled={page === 1} onClick={() => setPage((p) => p - 1)} className="border px-3 py-1 rounded disabled:opacity-50">Prev</button>
          {Array.from({ length: totalPages }, (_, i) => i + 1).map((p) => (
            <button key={p} onClick={() => setPage(p)} className={`border px-3 py-1 rounded ${p === page ? "bg-blue-500 text-white" : ""}`}>{p}</button>
          ))}
          <button disabled={page === totalPages} onClick={() => setPage((p) => p + 1)} className="border px-3 py-1 rounded disabled:opacity-50">Next</button>
        </div>
      )}
      ...
      {(showAddModal || showEditModal) && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
          <div className="bg-white rounded-lg shadow-lg w-full max-w-md p-6">
            <h2 className="text-lg font-bold mb-4">{showAddModal ? "Add Entry" : "Edit Entry"}</h2>
            <form onSubmit={showAddModal ? handleAddSubmit : handleEditSubmit} className="space-y-3">
              <select value={entryForm.type} onChange={(e) => setEntryForm({ ...entryForm, type: e.target.value as "IN" | "OUT" })} className="border rounded w-full px-3 py-2">
                <option value="IN">IN</option>
                <option value="OUT">OUT</option>
              </select>

              <input type="number" value={entryForm.amount} onChange={(e) => setEntryForm({ ...entryForm, amount: e.target.value })} className="border rounded w-full px-3 py-2" placeholder="Amount" required />

              <select value={entryForm.mode} onChange={(e) => setEntryForm({ ...entryForm, mode: e.target.value as "Cash" | "Online" })} className="border rounded w-full px-3 py-2">
                <option value="Cash">Cash</option>
                <option value="Online">Online</option>
              </select>

              <input type="text" value={entryForm.description} onChange={(e) => setEntryForm({ ...entryForm, description: e.target.value })} className="border rounded w-full px-3 py-2" placeholder="Description" />

              <div className="flex justify-end gap-2">
                <button type="button" onClick={() => { setShowAddModal(false); setShowEditModal(false); }} className="px-4 py-2 bg-gray-300 rounded">Cancel</button>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
          <div className="bg-white rounded-lg shadow-lg w-full max-w-sm p-6 text-center">
            ...
            <div className="flex justify-center gap-2">
              ...
              <button onClick={handleDeleteConfirm} disabled={submitting} className="px-4 py-2 bg-red-500 text-white rounded">
