...
import { ArrowUpDown, ChevronDown, ArrowLeft, FileText } from "lucide-react";
import {
  Select,
  SelectTrigger,
  SelectContent,
    ...
    printWindow.document.write(`
      <html>
        <head>
          ...
          <style>body{font-family:Arial,sans-serif;padding:20px}table{border-collapse:collapse;width:100%}table,th,td{border:1px solid #ddd; padding: 8px;}th,td{padding:8px;text-align:left}th{background-color:#f2f2f2}</style>
        </head>
    ...
    <div className="flex flex-col h-screen bg-gray-50">
      <header className="bg-white border-b border-gray-200 p-4 flex items-center">
        <div className="flex items-center text-lg font-semibold text-gray-800">
          <button onClick={() => router.back()} className="mr-2 p-1 rounded-md hover:bg-gray-100 cursor-pointer"><ArrowLeft className="h-5 w-5" /></button>
          Cashbook
        </div>
      </header>

      <main className="container mx-auto p-6 flex-1 flex flex-col overflow-hidden min-h-0">
          ...
          <div className="flex flex-wrap items-center justify-between gap-4 mb-6">
            <div className="flex flex-wrap gap-2 items-center">
              ...
              <Select value={filterType} onValueChange={(v: FilterType) => setFilterType(v)}>
                <SelectTrigger className="w-40 cursor-pointer" disabled={loading}><SelectValue placeholder="Date Range"/></SelectTrigger>
                <SelectContent>
              ...
              <Select value={typeFilter} onValueChange={(v) => setTypeFilter(v as any)}>
                <SelectTrigger className="w-40 cursor-pointer" disabled={loading}><SelectValue placeholder="Type"/></SelectTrigger>
                <SelectContent>
              ...
              <Select value={modeFilter} onValueChange={(v) => setModeFilter(v as any)}>
                <SelectTrigger className="w-40 cursor-pointer" disabled={loading}><SelectValue placeholder="Mode"/></SelectTrigger>
                <SelectContent>
                  <SelectItem value="all">
                    <div className="flex items-center gap-2">
                      <span className="w-2.5 h-2.5 rounded-full bg-gray-400"></span>
            ...
            <div className="flex items-center gap-3">
              ...
              <Button variant="outline" className="flex items-center gap-2 cursor-pointer" onClick={printTable} disabled={loading}>Print Cash Book <FileText className="w-4 h-4 ml-1" /></Button>
            </div>
          </div>

          {/* Report content (wrapped by pdfRef for printing) */}
          <div ref={setPdfRef} id="report-content" className="flex-1 flex flex-col min-h-0">
            ...
            <div className="flex-1 flex flex-col overflow-hidden min-h-0">
              <Table wrapperClassName="flex-1 min-h-0 overflow-auto" className="min-w-full w-full table-fixed text-sm">
                <colgroup>
                  <col style={{ width: '5%' }} />
                  <col style={{ width: '10%' }} />
                  <col style={{ width: '10%' }} />
                ...
                <TableHeader className="bg-gray-100 sticky top-0 z-20">
                  <TableRow>
                    <TableHead className="sticky top-0 bg-gray-100 z-20 w-20">S. No.</TableHead>
                    <TableHead onClick={() => handleSort("type")} className="cursor-pointer select-none">
                      <div className="flex items-center gap-1">Type <ArrowUpDown className={`w-4 h-4 transition-transform ${sortConfig.key === "type" ? (sortConfig.direction === "asc" ? "rotate-180 text-blue-600" : "text-blue-600") : "opacity-70"}`} /></div>
                    </TableHead>
                    <TableHead onClick={() => handleSort("mode")} className="cursor-pointer select-none">
                      <div className="flex items-center gap-1">Mode <ArrowUpDown className={`w-4 h-4 transition-transform ${sortConfig.key === "mode" ? (sortConfig.direction === "asc" ? "rotate-180 text-blue-600" : "text-blue-600") : "opacity-70"}`} /></div>
                    </TableHead>
                    <TableHead onClick={() => handleSort("date")} className="cursor-pointer select-none">
                      <div className="flex items-center gap-1">Date <ArrowUpDown className={`w-4 h-4 transition-transform ${sortConfig.key === "date" ? (sortConfig.direction === "asc" ? "rotate-180 text-blue-600" : "text-blue-600") : "opacity-70"}`} /></div>
                    </TableHead>
                    <TableHead>Description</TableHead>
                    <TableHead onClick={() => handleSort("amount")} className="cursor-pointer select-none text-right">
                      <div className="flex items-center gap-1 justify-end">Amount <ArrowUpDown className={`w-4 h-4 transition-transform ${sortConfig.key === "amount" ? (sortConfig.direction === "asc" ? "rotate-180 text-blue-600" : "text-blue-600") : "opacity-70"}`} /></div>
                    </TableHead>
                  </TableRow>
                </TableHeader>
                ...
                <TableBody>
                  {loading ? (
                    ...
                    <TableRow>
                      <TableCell colSpan={6} className="text-center py-10 text-gray-500">No entries found for the selected filters.</TableCell>
                    </TableRow>
                  ) : (
                    filtered.map((e, idx) => (
                      <TableRow key={e._id}>
                        ...
                        <TableCell>
                          <span className={`px-2 py-1 text-xs font-semibold rounded-full ${e.type === "IN" ? "bg-green-100 text-green-700" : e.type === "OUT" ? "bg-red-100 text-red-600" : "bg-gray-100 text-gray-700"}`}>
                            {String(e.type || "")}
                          </span>
                        </TableCell>
                        <TableCell>
                          <span className={`px-2 py-1 text-xs font-semibold rounded-full ${e.mode === "cash" ? "bg-green-100 text-green-700" : e.mode === "online" ? "bg-blue-100 text-blue-700" : "bg-gray-100 text-gray-700"}`}>
                            {capitalize(String(e.mode || ""))}
                          </span>
                        </TableCell>
                        <TableCell>{format(parseISO(e.createdAt), "dd/MM/yyyy")}</TableCell>
                        <TableCell className="whitespace-normal break-words">{e.description || "-"}</TableCell>
                        <TableCell className="text-right min-w-[120px]">₹{(e.amount || 0).toLocaleString()}</TableCell>
                      </TableRow>
                    ))
                  )}
                </TableBody>
