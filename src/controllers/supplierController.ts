...
//   return Supplier.find({ business: user.id, isDeleted: false }).sort({ createdAt: -1 });
// };

// // GET single supplier by ID
// export const getSupplierById = async (id: string, user: UserPayload): Promise<ISupplier | null> => {
//   const supplier = await Supplier.findById(id);
//   if (!supplier || supplier.isDeleted || supplier.business.toString() !== user.id) return null;
//   return supplier;
// };

// // CREATE new supplier
// export const createSupplier = async (body: Partial<ISupplier>, user: UserPayload): Promise<ISupplier> => {
...
//   // Ensure name and phone are not empty if present
//   if ("name" in updateData && !updateData.name) throw new Error("Supplier name is required");
//   if ("phone" in updateData && !updateData.phone) throw new Error("Supplier phone is required");

//   Object.assign(supplier, updateData, { updatedBy: user.id });
//   await supplier.save();
//   return supplier;
// };

// // DELETE (soft) supplier
// export const deleteSupplier = async (id: string, user: UserPayload): Promise<ISupplier | null> => {
//   const supplier = await Supplier.findById(id);
//   if (!supplier || supplier.isDeleted || supplier.business.toString() !== user.id) return null;
